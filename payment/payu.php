...
class Payment {
	...
	public function pay ( $params = null )
	{
		...
		if ( $error === true ) {
			...
			$keyValueArray = array();
			$keyValueArray['order_id'] = $params['txnid'];
			$keyValueArray['payment_status'] = 'success';
			$dataArr = $db-> getDataFromTable($keyValueArray, 'order', "id", '', '', false);
			if ( count($dataArr) > 0) return array ( 
				'status' => Misc::FAILURE, 
				'data' => $result['error'] );
